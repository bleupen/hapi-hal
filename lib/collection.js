...
Collection.prototype.toHal = function(rb, context, done) {
    ...
    var urlTemplate = template.parse(config.item && config.item.url || './{_id}');

    function embed(entity, cb) {
        var embedUrl = urlTemplate.expand(entity);
        rb.toHal(entity, path.resolve(url, embedUrl), config.item && config.item.config || {}, cb);
    }
    ...
    async.map(this.items, embed, function (err, results) {
        if (err) {
            ...
            var rel = config.item && config.item.rel ? rb.resolve(config.item && config.item.rel) : 'items';
            rep.embed(rel, results, false);
            done(null, rep);
        }
    });
};
